            ...
            &ASTNode::SQLSelect {
                ...
                let projection_schema = Schema {
                    columns: expr.iter().map( |e| match e {
                        ...
                        //TODO: hard-coded until I have function metadata in place
                        &Expr::ScalarFunction { ref name, .. } => Field::new(name, "f64", true),
                        _ => unimplemented!()
                    }).collect()
                };
    ...
    pub fn sql_to_rex(&self, sql: &ASTNode, schema: &Schema) -> Result<Expr, String> {
        match sql {
            ...
            &ASTNode::SQLLiteralInt(n) =>
                Ok(Expr::Literal(LiteralValue::Long(n as i64))), //TODO
...
//pub fn convert_data_type(sql: &SQLType) -> Box<DataType> {
//    match sql {
